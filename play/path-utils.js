...
import { extname, basename, dirname, normalize, absolute, relative, isRemote, isAbsolute, resolveSync } from "../src/DB/path.js"
...
export async function runPathUtilsDemo(console) {
	...
	console.info(tabbed(`basename("/dir/file.txt", ".txt") → ${JSON.stringify(basename("/dir/file.txt", ".txt"))}`))
	console.info(tabbed(`basename("/dir/file.txt", true) → ${JSON.stringify(basename("/dir/file.txt", true))}`))
	...
	console.info(tabbed(`absolute("/base", "root", "file") → ${JSON.stringify(absolute("/base", "root", "file"))}`))
	console.info(tabbed(`absolute("https://ex.com", "api", "v1") → ${JSON.stringify(absolute("https://ex.com", "api", "v1"))}`))
	...
	console.info(tabbed(`relative("/root/dir", "/root/") → ${JSON.stringify(relative("/root/dir", "/root/"))}`))

	await pressAnyKey(console)

	// Demo: resolveSync
	console.info("\n7. Synchronous path resolution:")
	console.info(tabbed(`resolveSync("/base", ".", "a/b/../c") → ${JSON.stringify(resolveSync("/base", ".", "a/b/../c"))}`))
